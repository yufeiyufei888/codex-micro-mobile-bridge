...
原生 Android V1.0.1 客户端，使用 Kotlin、Jetpack Compose Material 3 与单向数据流。它通过同一局域网内的证书绑定 WSS 连接 Windows Bridge；不开放 BLE、语音或 PTT。V1.0.1 延续已通过实机验收的桌面发送、断网重连、完整长回复、后台保活和手机审批链路，并新增可点击的完整对话历史页；顶部状态卡只表达状态，不再重复回复正文。
...
- “桌面权限确认”展示当前对话的权限请求。批准仍需长按至少 600ms；Bridge 会在执行前重新核对审批身份，避免批准已经变化的请求。
