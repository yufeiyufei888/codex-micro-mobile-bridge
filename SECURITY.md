...
当前稳定版本为 v1.0.6。它已完成自动验证和关键联合实机验收，但只保证单一当前桌面对话的基础链路；多个桌面对话同时活动时可能发生归属混流。`main` 上的 v2.0.0 是自动门禁通过、真实联合验收待完成的候选源码，不是正式稳定 Release。v1.1.0 与 v1.1.1 是历史候选归档，v1.0.5 存在严重同步回归。安全问题统一在新的补丁版本中处理，不回写或移动历史标签。
